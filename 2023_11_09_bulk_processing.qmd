...
I will want to use GeneTonic for pathway analysis, so we will start some enrichment analysis.

First to filter to only significant genes:

```{r}
mix.4.b.vs.rej.4.anno.enrich <- mix.4.b.vs.rej.4.annotation.filtered |> 
  filter(padj <= 0.05) |> 
  pull(gene_symbol)
```

Next we need the background genes from the comparison

```{r}
bg_ids <- deseqresult2df(mix.4.b.vs.rej.4.res) |>  
...
```{r}
mix.4.b.vs.rej.4.topgo = pcaExplorer::topGOtable(mix.4.b.vs.rej.4.anno.enrich,
                                              bg_ids,
                                              ontology = "BP",
...
mix.4.b.vs.rej.4.topgo = shake_topGOtableResult(mix.4.b.vs.rej.4.topgo)
mix.4.b.vs.rej.4.topgo = get_aggrscores(mix.4.b.vs.rej.4.topgo, mix.4.b.vs.rej.4.res, mix.4.b.vs.rej.4.annotation.filtered)
```
...
GeneTonic(dds = sf.df.dds,
          res_de = mix.4.b.vs.rej.4.res,
          res_enrich = mix.4.b.vs.rej.4.topgo,
          annotation_obj = sf.df.annotation,
          project_id = "TEST1")
```
```{r}
deseqresult2df(mix.4.b.vs.rej.4.res) %>% 
  dplyr::rename(gene_id = id) %>% 
