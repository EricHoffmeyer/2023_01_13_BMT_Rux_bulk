...
heatmap.name <- paste0(folder.name, "/heatmaps/")

if(!exists(heatmap.name)){
  dir.create(heatmap.name)}

volcano.name <- paste0(folder.name, "/volcano_plots/")
                       
if(!exists(volcano.name)){
  dir.create(volcano.name)}

summary.name <- paste0(folder.name, "/summary_plots/")
                       
if(!exists(summary.name)){
  dir.create(summary.name)}
```
...
Creating a function where:
  con1 = condition 1
  con2 = condition 2
  name1 = abbreviation of condition 1 for file names
  name2 = abbreviation of condition 2 for files names
...
deseq.df |> 
  ...
  pheatmap(scale = "row",
         ...
         annotation_colors = anno_colors,
         main = "Top 25 genes by p-value") |> 
         ggsave(filename = paste0(heatmap.name, name1, ".VS.", name2, "log2FC.png"))
