...
![](Code%20Review%20Figures/Mouse%20Design.png)

This markdown will walk through the process behind three of my other notebooks: `2023_11_09_bulk_processing`, `2024_01_10_Condition_DESeqs`, `2024_01_19_Condition_Genetonic.qmd`
...
The original script that did was created 2024_01_10. Keeping names consistent for this walkthrough.
...
In a smaller bulk RNAseq dataset, where you have only one or two conditions, the counts table would be filtered for low reads as the next step.

However, this dataset has multiple cell types and other variables like mouse strain (H2Kb/H2Kd).

I want to create a subset that contains the appropriate columns for my comparison of interest before filtering for reads.
...
This comparison will use the following: condition 1 : "Mixed CD4 HLA B" condition 2 : "Reject CD4"

I will use shorthand versions of these groups when writing results files: name 1 : "mix.4.b" name 2 : "rej.4.b"
...
deseq.df |> 
  ...
  pheatmap(scale = "row",
         ...
         ggsave(filename = paste0(heatmap.name, "mix.4.b.VS.rej.4.b.padj.png"))
```
...
deseq.df |> 
  ...
  pheatmap(scale = "row",
         ...
         ggsave(filename = paste0(heatmap.name, "mix.4.b.VS.rej.4.b.log2FC.png"))
```
...
EnhancedVolcano(toptable = deseq.df,
                ...
                legendPosition = "none",
                caption = "") |> 
  ggsave(filename = paste0(volcano.name, "mix.4.b.VS.rej.4.b.volcano.png"))
```
...
ggsave(filename = paste0(summary.name, "mix.4.b.VS.rej.4.b.summary.png"),
       plot = p,
       width = 11,
       height = 8,
       units = "in",
