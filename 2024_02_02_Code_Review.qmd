...
The only changes are the condition variables are called con1 and con2, and we use the name1 and name2 variables with a `paste0()` function to export the results to the correct folders.
...
Some words here aboust using pmap vs lapply, mapply, etc...
...
But that is hard to navigate. I think it would be better for each comparison to have its own folder that contains the relevant plots. We'll let the function do that for us as well.

We will need to read in the data for the plots. We technically have it saved in our variables for this sheet, but doing it this way helps us fix any issues when we apply the process to the rest of the comparisons.
...
The column names in the counts matrix provide the mouse number but are quite long and hard to interpret Making an annotation dataframe will make the heatmaps more accessible.
