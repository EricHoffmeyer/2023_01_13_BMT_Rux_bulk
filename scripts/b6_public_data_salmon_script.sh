...
INDEX1=$GENOMES/STAR_indices/murine_GRCm39_Ensembl_104_w_decoy_and_w_nrcrna
INDEX2=$GENOMES/Salmon_indices/murine_GRCm39_Ensembl_104_w_decoy_and_w_nrcrna
DATA=$RNASEQ/${INVESTIGATOR}/$PROJECT
SCRIPTS=$DATA/scripts
FASTQ=$DATA/fastq_files/public_data
...
for FILE in `ls *.fastq.gz | cut -d "." -f 1`; # | cut -d "_" -f 1,2,3,4,5`;
...
bbduk.sh -Xmx38g \
    threads=10 \
    in1=$FASTQ/${FILE}.fastq.gz \
    out1=$TRIMMED/${FILE}.fastq.gz \
...
# The -r flag is for single end reads
